#!/usr/bin/env python3
"""
Wayland Testing Agent - Captures visual output and converts to numerical metrics.
...
class WaylandTestAgent:
    """Test agent for Wayland display server."""
    
    def __init__(self, target_pid, output_file="test_results_wayland.json"):
    ...
    def capture_screen_wayland(self):
        """Capture screen using Wayland tools (grim or gnome-screenshot)."""
        try:
            # Try grim first (wlroots compositors: sway, etc.)
            result = subprocess.run(
                ['grim', '/tmp/test_capture_wayland.png'],
                capture_output=True,
                timeout=2
            )
            if result.returncode == 0:
                img = Image.open('/tmp/test_capture_wayland.png')
                return np.array(img)
        except (FileNotFoundError, subprocess.TimeoutExpired):
            pass
        
        try:
            # Try gnome-screenshot (GNOME on Wayland)
            result = subprocess.run(
                ['gnome-screenshot', '-f', '/tmp/test_capture_wayland.png'],
                capture_output=True,
                timeout=2
            )
            if result.returncode == 0:
        ...
        try:
            # Try spectacle (KDE Plasma on Wayland)
            result = subprocess.run(
                ['spectacle', '-b', '-n', '-o', '/tmp/test_capture_wayland.png'],
                capture_output=True,
                timeout=2
            )
            if result.returncode == 0:
    ...
    def run(self):
        ...
        print(f"[Wayland Test Agent] Starting test for PID {self.target_pid}")
        print(f"[Wayland Test Agent] Will run for {self.test_duration} seconds")
        print(f"[Wayland Test Agent] Capturing every {self.capture_interval} seconds")
        
        sample_count = 0
        
        while time.time() - self.start_time < self.test_duration:
            # Capture screen
            frame = self.capture_screen_wayland()
            ...
            if frame is not None:
                ...
                if metrics:
                    ...
                    print(f"[Wayland Test Agent] Sample {sample_count} @ {elapsed:.1f}s: "
                          f"Red={'YES' if red_detected else 'NO'} "
                          f"Coverage={red_coverage:.2f}%")
            else:
                print(f"[Wayland Test Agent] Failed to capture frame at {time.time() - self.start_time:.1f}s")
            
            # Wait for next sample
            time.sleep(self.capture_interval)
        
        # Test complete - kill target process
        print(f"\n[Wayland Test Agent] Test duration complete")
        self.save_results()
        self.kill_target()
    ...
    def save_results(self):
        ...
        print(f"[Wayland Test Agent] Results saved to {self.output_file}")
    ...
    def print_summary(self, summary):
        ...
        print("\n" + "=" * 60)
        print("WAYLAND TEST RESULTS SUMMARY")
        print("=" * 60)
    ...
    def kill_target(self):
        ...
        try:
            print(f"[Wayland Test Agent] Sending SIGTERM to PID {self.target_pid}")
            ...
            if result.returncode == 0:
                print(f"[Wayland Test Agent] Process still running, sending SIGKILL")
                subprocess.run(['kill', '-KILL', str(self.target_pid)], timeout=1)
        except Exception as e:
            print(f"[Wayland Test Agent] Error killing target: {e}")


def main():
    """Entry point for Wayland test agent."""
    if len(sys.argv) < 2:
        print("Usage: test_agent_wayland.py <target_pid>")
        sys.exit(1)
    
    target_pid = int(sys.argv[1])
    output_file = sys.argv[2] if len(sys.argv) > 2 else "test_results_wayland.json"
    
    agent = WaylandTestAgent(target_pid, output_file)
    agent.run()
